...
/**
 * Subclass for delta strategy gametype's rules
 * @author Ransom and Chrissy
 * @version 1.0
 */
public class DeltaStrategyGameController extends CommonStrategyGameController {
	...
	public MoveResult move(PieceType piece, Location from, Location to) throws StrategyException {
		...
		if (piece == PieceType.BOMB) {
			throw new StrategyException("You can't move a bomb!");
		}
		...
		if (!((((from.getCoordinate(Coordinate.X_COORDINATE) + 1) == to.getCoordinate(Coordinate.X_COORDINATE)) 
				...
				|| (((from.getCoordinate(Coordinate.Y_COORDINATE) + 1) == to.getCoordinate(Coordinate.Y_COORDINATE)) 
						|| ((from.getCoordinate(Coordinate.Y_COORDINATE) - 1) == to.getCoordinate(Coordinate.Y_COORDINATE)))) && piece != PieceType.SCOUT) {
		...
		if (!((((from.getCoordinate(Coordinate.X_COORDINATE) + 1) == to.getCoordinate(Coordinate.X_COORDINATE)) 
			...
			if (getPieceAt(to) != null) {
				throw new StrategyException("Your scout is trying to move more than one location AND attack");
			}
		}
